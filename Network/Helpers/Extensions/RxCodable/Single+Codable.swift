...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == Data {
...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == String {
    public func map<T>(_ type: T.Type, using decoder: JSONDecoder? = nil) -> PrimitiveSequence<TraitType, T> where T: Decodable {
        return self
